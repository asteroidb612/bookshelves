...
main =
    program
        { init = NoShelf ! [ generateRandomShelf ]
        , update = update
        , view = renderedShelf
...
update msg shelf =
    case (msg) of
        ChangeShelf ->
            shelf ! [ generateRandomShelf ]

        NewShelf b ->
            b ! []


generateRandomShelf : Cmd Msg
generateRandomShelf =
    let
        randomBook =
            Random.int 0 121
    in
        Random.generate NewShelf (Random.map ShelfWithSevenRandomBooks (Random.list 7 randomBook))
...
--Grabbed from some list on Wikipedia


bookTitles : Array.Array String
bookTitles =
    Array.fromList
        [ "Kanokon"
        , "Kara no Kyōkai"
